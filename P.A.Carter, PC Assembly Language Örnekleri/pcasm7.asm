; pcasm7.asm: Büyük tamsayý toplama ve çýkarma ASM altprogram örneði.
;
...
;---------------------------------------------------------------------------------------------------------------------
; NOT: gcc ile .cpp C dosyalarý link yapýlamýyor, cl gerekiyor.
;---------------------------------------------------------------------------------------------------------------------
;
; C'de yapýsal deðiþkenler topluca alt (karma tipli) deðiþkenleri kapsar:
; struct S {
; short int x; /* 2-byte'lý tamsayý */
; int y; /* 4-byte tamsayý */
; double z; /* 8-byte kayan */
; }; sona gcc paketli yapýsallýk vasfý da konabilir:
; } attribute ((packed));
;
; direktiflerle yapýsal yerleþimi 1:byte, 2:kelime, 4:duble kelime, 8:dörtlü kelime, 16:paragraf
; #pragma pack(push) /* mevcut yerleþim durumu saklanýr */
; #pragma pack(1) /* byte yerleþim ayarlanýr */
; struct S {
; short int x; /* 2-byte tamsayý */
; int y; /* 4-byte tamsayý */
; double z; /* 8-byte kayan */
; };
; #pragma pack(pop) /* ilk yerleþim geçerlilenir */
;
; Bit yapýsal alan yerleþimi de araya : kolon iþaretiyle mümkündür:
; struct S {
...
; };
;
; Class/sýnýf, içinde hem veriler hemde fonksiyonlar bulunabilen yapýdýr.
; class Basit {
; public:
; Basit (); // varsayýlý kurucu
; "Basit(); // yýkýcý
; int al_veri () const; // üye fonksiyonlar
; void koy_veri ( int );
; private:
; int veri; // üye veri
; };
